...
**Scaffold build queue:** [done] UI primitives · [done] tour-type/section routes · [done] dashboard shells · [done] DB schema + CMS collections (build-isolated) · [done] booking-flow UI + payment-pending state (`/book`, `/api/bookings`) · [done] customer journey — booking tracking (`/track`, `/api/bookings/lookup`, reusable `BookingStatusTimeline`) · [next] quote generation refinement / CRM workflow (payment-independent priorities 3–10) · [ ] apply primitives across existing pages (A5) · [ ] A3 hero + A4 mobile polish.
