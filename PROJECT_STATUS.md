...
**Scaffold build queue:** [done] UI primitives · [done] tour-type/section routes · [done] dashboard shells (hub/vendor/crm/admin) · [next] DB schema + CMS collections (build-isolated, needs `DATABASE_URL` to go live) · [ ] booking-flow UI expansion · [ ] apply primitives across existing pages (A5) · [ ] A3 hero + A4 mobile polish.
