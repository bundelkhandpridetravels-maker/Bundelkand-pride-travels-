...
**Scaffold build queue:** [done] UI primitives · [done] tour-type/section routes · [done] dashboard shells (hub/vendor/crm/admin) · [done] DB schema + CMS collections (build-isolated in `src/payload/`, 19 collections, local Payload-mirror types, no deps/DATABASE_URL) · [next] booking-flow UI expansion · [ ] apply primitives across existing pages (A5) · [ ] A3 hero + A4 mobile polish.
